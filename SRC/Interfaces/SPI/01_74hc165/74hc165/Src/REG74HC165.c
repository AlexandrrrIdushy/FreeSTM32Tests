...
void load_config_74HC165(struct reg74hc165_current_state *current_state)
{
	...
	for (i = 0; i < NUM_OF_INPUT; i++)
	{
		current_state->arr_res[i].config.i8 = 1;// = FLASH_Read_Config_Byte(i);
	}
}
...
void init_74HC165(struct reg74hc165_current_state *current_state)
{
	...
	for (i = 0; i < NUM_OF_INPUT; i++)
	{
		...
		current_state->arr_res[i].status.bf.cur_phis_state =
				~current_state->arr_res[i].config.bf.alarm_state; // что-бы при старте не были выданы ложные срабатывания
	}
}
...
void meander_recognition(struct reg74hc165_current_state *current_state,
		int input_num)
{
	if ((current_state->arr_res[input_num].pulse_duration > NOICE_DURATION)
			&& (current_state->arr_res[input_num].pulse_duration
					< THRESHOLD_DURATION))
	{
		...
		int input_num)
{
	...
	if ((current_state->arr_res[input_num].pause_duration >= NOICE_DURATION)
			&& (current_state->arr_res[input_num].status.bf.is_const_sig == 1))
	{
...
void pulse_processing(struct reg74hc165_current_state *current_state)
{
	...
	for (i = 0; i < NUM_OF_INPUT; i++)
	{
		...
		{
			if (current_state->arr_res[i].pulse_duration
					<= THRESHOLD_DURATION + 1) // смысл инкрементировать длительность импульса больше чем THRESHOLD_DURATION нет
			{
...
//					current_state->arr_res[NUM_OF_INPUT - 1
//							- current_state->current_bit].status.bf.cur_phis_state;
//		}
//		else
//		{
//			current_state->arr_res[NUM_OF_INPUT - 1 - current_state->current_bit].status.bf.cur_log_state =
...
//		if (QH_PIN_STATE) // Вычитываем текущий бит
		...
		else
		{
			current_state->arr_res[NUM_OF_INPUT - 1 - current_state->current_bit].status.bf.cur_phis_state =
					0;
			if((NUM_OF_INPUT - 1 - current_state->current_bit) == 12)
				asm("nop");
		}
